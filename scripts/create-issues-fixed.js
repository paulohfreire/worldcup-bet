 ...
 * 1. Com GitHub CLI (gh) - cria issues automaticamente
 * 2. Sem GitHub CLI - gera arquivo de issues manualmente
 *
 * Uso:
 *   node scripts/create-issues-fixed.js [--auto]
 *
 * --auto: Força tentar usar GitHub CLI (mesmo sem verificar se está instalado)
 */
...
function checkGhCli() {
  try {
    ...
    const auth = execSync('gh auth status', { stdio: 'pipe', encoding: 'utf-8' });
    return !auth.includes('not logged');
  } catch (error) {
    return false;
  }
}

// Lista de tarefas do PLAN.md organizadas por agente
const TASKS = [
  // Infra tasks
  {
...
/**
 * Cria uma issue no GitHub usando CLI
 */
async function createIssueWithCLI(task, index) {
  try {
    ...
    // Retornar o número da issue (extrair da URL)
...
/**
 * Gera arquivo Markdown com issues para criação manual
 */
...
async function main() {
  ...
  const forceAuto = args.includes('--auto');

  console.log('\n🚀 Gerando issues do PLAN.md para o GitHub...\n');

  const hasGhCLI = checkGhCli();

  if (hasGhCLI || forceAuto) {
    console.log('✅ GitHub CLI encontrado e autenticado\n');
    ...
    console.log('⚠️  GitHub CLI não encontrado ou não autenticado\n');
    console.log('Gerando arquivo de issues para criação manual...\n');
    ...
    console.log('   3. OU instale o GitHub CLI e execute novamente com --auto\n');
  }
}
