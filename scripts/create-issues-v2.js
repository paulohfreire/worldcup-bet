 ...
 * Versão melhorada que encontra GitHub CLI no Windows
 *
 * Uso:
 *   node scripts/create-issues-v2.js
 */
...
function findGitHubCLI() {
  // Tenta usar PATH primeiro
  try {
    execSync('gh --version', { stdio: 'pipe', timeout: 5000 });
    return 'gh';
  } catch (error) {
    // gh não está no PATH, tenta encontrar
  }

  // Tenta os caminhos específicos
  for (const path of GITHUB_CLI_PATHS) {
    if (path === 'gh') continue;

    let testPath = path;
    // Substituir $USER pelo username real
...
function checkGitHubAuth(ghPath) {
  try {
    ...
    // Se não houver erro, assume que está logado
    return !result.error;
  } catch (error) {
    return false;
  }
...
/**
 * Cria uma issue no GitHub usando o caminho completo do gh CLI
 */
function createIssue(ghPath, task, index) {
  try {
    ...
    // Escapar aspas simples no corpo
    const escapedBody = bodyWithDeps.replace(/'/g, "\\'");

    const command = `"${ghPath}" issue create --title "${task.title}" --body '${escapedBody}' --label "${labelString}"`;
    ...
    if (error.message.includes('Stderr')) {
      console.error(`   Detalhes: ${error.message.split('Stderr:')[1]?.trim()}`);
    }
    return null;
  }
}
...
async function main() {
  ...
  if (!ghPath) {
    ...
    console.log('   3. OU instale o GitHub CLI em um local que o Git Bash possa encontrar\n');
    return;
  }
