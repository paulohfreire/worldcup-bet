...
/**
 * Script para criar issues no GitHub com opção de login integrado
 *
 * Uso:
 *   node scripts/create-issues-with-login.js
 */
...
async function main() {
  ...
  if (!ghPath) {
    console.log('❌ GitHub CLI não encontrado\n');
  ...
  if (!isAuthenticated) {
    ...
    const doLogin = await askQuestion('Deseja fazer login agora? (S/N): ');

    if (doLogin.toLowerCase() === 's' || doLogin.toLowerCase() === 'sim') {
      console.log('\n🔄 Tentando fazer login...\n');

      try {
        const loginResult = spawnSync(ghPath, ['auth', 'login', '--web'], {
          stdio: 'inherit',
          timeout: 60000
        });
        ...
        if (loginResult.status !== 0) {
          ...
          console.log('   2. Execute: gh auth login --web');
    ...
    // Verificar autenticação novamente após login
    const isNowAuthenticated = checkGitHubAuth(ghPath);
    if (!isNowAuthenticated) {
      console.log('❌ Ainda não autenticado. Por favor, tente novamente.');
      return;
    }
  } else {
